...
__global__ void single_iteration_gpu(float *result, float *temp, float *power, int row, int col, float Cap_1, float Rx_1, float Ry_1, float Rz_1, float step){
...
float compute_tran_temp_parallel(float *result, int num_iterations, float *temp, float *power, int row, int col) {
    int i = 0;
    ...
    cudaMalloc(&device_t, row * col * sizeof(float));
    cudaMalloc(&device_r, row * col * sizeof(float));
    cudaMalloc(&device_power, row * col * sizeof(float));
    cudaMemcpy(device_power, power, row * col * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(device_r, r, row * col * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(device_t, t, row * col * sizeof(float), cudaMemcpyHostToDevice);
    ...
    for (i = 0; i < num_iterations; i++) {
        ...
        dim3 grid_size((col + BLOCK_SIZE - 1) / BLOCK_SIZE, (row + BLOCK_SIZE - 1) / BLOCK_SIZE);

        single_iteration_gpu<<<grid_size, block_size>>>(device_r, device_t, device_power, row, col, Cap_1, Rx_1, Ry_1, Rz_1, step);
        
        // cudaMemcpy(r, device_r, row * col * sizeof(float), cudaMemcpyDeviceToHost);
        // cudaMemcpy(t, device_t, row * col * sizeof(float), cudaMemcpyDeviceToHost);
...
float parallel_execution(int argc, char **argv){
    ...
    // clock_t start = clock();
    float elapsed = compute_tran_temp_parallel(result, sim_time, temp, power, grid_rows, grid_cols);
...
float sequential_execution(int argc, char **argv){
    int grid_rows, grid_cols, sim_time;
    float *temp, *power, *result;
    char *tfile, *pfile, *ofile;
...
int main(int argc, char **argv) {
    ...
    float elapsedParallel = parallel_execution(argc, argv);
    float elapsedSequential = sequential_execution(argc, argv);

    printf("\nTime elapsed in ms(sequential execution ): %lf\n", elapsedSequential);
    printf("\nTime elapsed in ms(parallel execution ): %lf\n", elapsedParallel);
